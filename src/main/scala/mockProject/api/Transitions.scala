...
case class MockProps(when : List[String], what : String, delayOption : Option[FiniteDuration])
...
object Transition {
    ...
    implicit class TransitionAfterWhenAndWhatDefined(whenAndWhatDefined: MockDefinition) {
        def withDelay(finiteDuration: FiniteDuration): DelayedMockDefinition = new DelayedMockDefinition {
    ...
    implicit def MockDefinitionToMockProps(mockDefinition: MockDefinition) : MockProps =
    {
        MockProps(mockDefinition._when, mockDefinition._what, None)
    }

    implicit def DelayedMockDefinitionToMockProps(delayedMockDefinition: DelayedMockDefinition) : MockProps =
    {
        MockProps(delayedMockDefinition.when, delayedMockDefinition.what, Some(delayedMockDefinition._delay))
    }
}
